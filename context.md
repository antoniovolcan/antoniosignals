...
**Candidatos #2 y #3 — probados 2026-08-11 con simulador corregido (importa `teamWinProbability`/`calibrateWinProbability`/`applyThinSampleDampening`/`log5` directo de `lib/signals.js` en vez de reimplementarlos, precisamente para no repetir el bug de arriba), baseline verificado exacto contra run #50 (55.99%) antes de probar nada — AMBOS DESCARTADOS:**
- **#2 — blend de split casa/visitante del `teamLedger` en `recordWinPct`** (`homeHomeWinPct`/`awayAwayWinPct`, datos ya capturados en run #50, nunca antes usados como input): plano a negativo ya en mayo mismo (peso 0.15 → MayFull 54.50%, igual al baseline 54.50%; pesos mayores lo empeoran monótonamente hasta 52.07% en 0.35) — descartado sin llegar a probar contra la temporada completa, tal como pide la metodología.
- **#3 — sweep de `TEAM_RECORD_RECENT_WEIGHT`** (hoy 0.3, nunca barrido independientemente; blend de `homeSeasonWinPct`/`homeRecentWinPct` ya capturados): en mayo, `recentWeight=0.5` mejoraba limpio en ambas mitades (55.90%/54.63% vs baseline 55.90%/53.24%, MayFull 55.23% vs 54.50%). **Al correr contra la temporada completa (run #50, mismos datos, simulador verificado) el efecto se disolvió**: 55.95% (0.5) y 55.83% (0.4) vs baseline 55.99% — Mar-Jun casi sin cambio, Jul-Sep levemente peor en ambos casos. Confirma que la ganancia de mayo era ruido de ese mes específico, no señal real — exactamente el patrón que la metodología de "un cambio a la vez + verificar contra toda la temporada" está diseñada para atrapar.

Ningún candidato nuevo sobrevivió esta ronda. Modelo en producción sin cambios: **55.99% accuracy de moneyline, temporada completa**.

**Búsqueda agresiva multi-parámetro (2026-08-11/12), en respuesta al pedido de empujar mayo a 70-75%:** barrido aleatorio de 200,000 combinaciones simultáneas sobre 9 parámetros del modelo (`homeFieldBonus`, `recordAdjScale`, clamps, `careerWeight`, `recentWeight`, `shrink`, umbral/shrink de muestra chica) más Pitágoras y racha agregados al barrido — techo real encontrado: **~61.5-61.8% en mayo**, muy por debajo de 70-75%. Conclusión: no es un problema de tuning, es un techo estructural de esta arquitectura (log5 + ERA/récord/ofensiva) — para superarlo hace falta memorizar identidad de equipo (ya sabemos que colapsa fuera de muestra, 50.9%, ver el experimento del "modelo secundario" más abajo) o señales genuinamente nuevas.

**Candidato #4 — fatiga de bullpen (2026-08-12), DESCARTADO:** se construyó infraestructura real (no solo búsqueda de parámetros): `sumBullpenOutsFromBoxscore` (`lib/mlb.js`, con tests) suma outs de relevistas de un boxscore ya jugado (excluye al abridor vía `gamesStarted===1`), y `computeBullpenFatigueIP` (`scripts/backtest.js`) agrega esto en una ventana móvil de 3 días por equipo, cacheado por `gamePk`. Se corrió un backtest de mayo 2025 instrumentado (run #52) y se validó con el simulador verificado (baseline exacto 54.99% contra run #52). **Resultado: cualquier peso de la señal (0.01 a 0.12, en ambas direcciones de signo) empeora mayo respecto al baseline** — no hay ambigüedad, ni siquiera llegó a mejorar mayo para justificar probar contra la temporada completa. Se investigaron dos fuentes de datos adicionales por la misma vía (llamadas reales a la API de MLB, no solo documentación):
- **Lesiones/IL**: el endpoint `transactions` sí tiene datos reales e históricos por equipo/fecha (confirmado con Yankees mayo 2025), pero es texto libre sin estructurar — requeriría parsear descripciones para identificar jugador/rol/fecha. No se construyó código todavía.
- **Clima**: descartado para el modelo EN VIVO — el campo `weather` de la API de MLB está vacío antes del primer lanzamiento (confirmado con un juego "Scheduled" real), solo se llena cerca/durante el juego, así que no está disponible cuando el bot genera las señales del día. Usarlo en el backtest sería fuga de datos. Haría falta un servicio de pronóstico externo (ej. Open-Meteo) para ser legítimo, y de todos modos pega más en totales que en moneyline.
- La infraestructura de bullpen (`sumBullpenOutsFromBoxscore`, `computeBullpenFatigueIP`) se dejó en el código (con tests) aunque el candidato se descartó, mismo criterio que con `teamLedger`/Pitágoras/racha — instrumentación reutilizable, no wireada a producción.

**Candidato #5 — conteo de jugadores en la lista de lesionados (2026-08-12), DESCARTADO:** se construyó `fetchTeamTransactions` + `computeActiveILCount` (`lib/mlb.js`, con tests) — esta última reproduce cronológicamente las transacciones de un equipo (endpoint `transactions`, que sí tiene `person.id` estructurado y texto libre con "placed on the X-day injured list" / "activated from... injured list" / "transferred... to the Y-day injured list") para reconstruir cuántos jugadores están activos en la IL a una fecha de corte, sin fuga de datos (solo transacciones estrictamente anteriores). Validado contra transacciones reales de los Yankees antes de confiar en el regex.
- **Mayo (run #53) mejoraba limpio**: `scale=0.03, cap=0.20` (ajuste directo sobre la probabilidad, proporcional a la diferencia de jugadores lesionados entre ambos equipos) daba May1-15 empatado (55.90%=55.90%) y May16-31 mejor (57.87% vs 54.17%), MayFull 56.93% vs baseline 54.99%.
- **Temporada completa (run #54) NO aguantó — de hecho fue mucho peor que solo plano**: Mar-Jun 53.88% vs baseline 56.18%, Jul-Sep 51.93% vs baseline 55.79%, temporada completa 52.95% vs baseline 55.99%. Ambas mitades empeoraron con claridad. La ganancia de mayo era sobreajuste a ese mes específico, igual que el candidato de peso récord reciente/temporal — pero acá el efecto fuera de muestra fue una regresión fuerte, no solo un aplanamiento.
- Infraestructura (`fetchTeamTransactions`, `computeActiveILCount`, captura en `factors`) se dejó en el código con tests, sin wirear a producción — mismo criterio que bullpen/teamLedger.

**Cinco candidatos probados bajo la metodología "un cambio a la vez", cinco descartados**: racha, split casa/visitante, peso récord reciente/temporal, fatiga de bullpen, conteo de lesionados. El modelo en producción sigue sin cambios desde el 55.99% de moneyline validado el 2026-08-11. Patrón que se repite: candidatos basados en datos de mayo-only tienden a sobreajustarse a ese mes específico — cualquier candidato nuevo DEBE pasar la prueba de temporada completa antes de considerarse real, sin excepción.

**Experimento — auto-ajuste diario de parámetros ("¿el modelo aprende de sus errores día a día?", 2026-08-15), DESCARTADO:** el usuario preguntó explícitamente si el modelo aprende de sus errores en producción. Respuesta corta: no, hoy es 100% estático (los 7 parámetros de moneyline —`homeFieldBonus`, `recordAdjScale`, `clampLo`/`clampHi`, `careerWeight`, `recentWeight`, `shrink`— vienen fijos de `PROD` y nunca cambian con nuevos resultados). Para medir si *debería* aprender, se simuló localmente (script descartable `candidate_self_adjust.js`, nunca comiteado a producción) un walk-forward real sobre los mismos 2427 juegos del run #50: cada día se predice con los parámetros vigentes (así se mide el accuracy, sin fuga), y recién después se da un paso de descenso por gradiente sobre el Brier score del día (gradiente numérico, épsilon=1e-4, con límites duros por parámetro) antes de pasar al día siguiente. Un smoke test con `learningRate=0` reprodujo el baseline estático exacto (55.995%) bit a bit, confirmando que el loop no tiene bugs.
- **`learningRate=0.001` (ajuste muy suave): 56.04% temporada completa** (Mar-Jun 55.94%, Jul-Sep 56.14%) — apenas +0.04pp sobre el baseline de 55.99%, dentro del ruido para n=2427. Los parámetros derivan muy levemente alrededor de los valores de producción y nunca se alejan mucho (`homeFieldBonus` termina en 0.084 vs el 0.09 de partida).
- **`learningRate=0.005`: 55.34% temporada completa** (Mar-Jun 55.78%, Jul-Sep 54.85%) — **peor** que el baseline, y notablemente peor en la segunda mitad. Los parámetros derivan de forma sostenida hacia abajo: `homeFieldBonus` 0.09→0.069, `shrink` 0.50→0.486, sin volver a converger.
- **`learningRate=0.02` (agresivo): 55.21% temporada completa** (Mar-Jun 56.02%, Jul-Sep 54.33%) — el peor de los tres, y con oscilación visible en vez de convergencia: `homeFieldBonus` salta 0.09→0.128→0.082→0.043→0.059→0.065→0.055 en muestras cada ~30 días en las muestras de trayectoria, sin asentarse. La mitad Jul-Sep se degrada ~1.5pp respecto al baseline Jul-Sep (55.79%).
- **Conclusión**: el auto-ajuste diario no ayuda. En el mejor caso (paso minúsculo) el efecto es indistinguible de ruido (+0.04pp); en cualquier paso con magnitud real, los parámetros se alejan de los valores ya optimizados por barrido completo de temporada (ver arriba) y el accuracy cae, empeorando más en la segunda mitad de temporada que en la primera — el gradiente diario persigue ruido de muestra chica (una fecha de juegos, ~13 partidos) en vez de señal real. Confirma con datos que la arquitectura actual no debe auto-ajustarse en vivo; la forma correcta de mejorar el modelo sigue siendo el proceso ya validado (barrido sobre temporada completa, un cambio a la vez, ver metodología arriba), no una corrección de gradiente día a día. `candidate_self_adjust.js` fue un script descartable y se eliminó tras esta corrida — no quedó wireado a producción.
