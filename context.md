...
**Patrones encontrados pero no atacados todavía (candidatos para la próxima sesión)**: calibración de moneyline mejoró pero no se cerró del todo (favorito visitante <40% real ~50.6% vs. predicho ~37-43%); K-rate individual del lineup rival en ponches (la libreta `battingLedger.js` ya existe para OPS, se podría extender); clima solo se aplica a ponches, no a totales; sin ajuste por descanso/fatiga del pitcher/equipo; OPS de carrera del bateador sigue sin implementar.

**Experimento descartado — bullpen en totales, vía proxy de temporada anterior (NO implementado, revertido)**: se probó blendear el ERA del abridor con el ERA del bullpen del equipo (ponderado por cuántas entradas tira cada uno, ~5.5/9 abridor + ~3.5/9 bullpen) en `projectedTotalRuns` (nunca en F5, ahí el bullpen todavía no entra). El bullpen SÍ se puede aislar en la API de MLB (`stats=statSplits&group=pitching&sitCodes=rp`, confirmado con datos reales: ERA de bullpen distinto del ERA de todo el staff, `gamesStarted: 0`) — pero **ese filtro se ignora silenciosamente al combinarlo con `byDateRange`** (mismo bug que ya conocíamos para bateo por mano a nivel de jugador), así que no hay forma confiable de pedir "el bullpen tal como estaba en una fecha pasada" para el backtest. Se probó con la temporada ANTERIOR completa como proxy leak-free (mismo truco que el ERA/K9 de carrera del abridor). Resultado en junio 2026 solo: mejora chica (MAE 3.532→3.501). Resultado en la **temporada 2025 completa**: la mejora se diluyó a casi nada (MAE 3.649→3.648, bias -0.111→-0.110) — ruido de un mes, no señal real. **Conclusión**: el proxy de temporada anterior probablemente está demasiado desactualizado (no capta trades/lesiones/ascensos dentro de la temporada) para aportar algo; si se quiere retomar, habría que construir un "bullpen ledger" desde jugada-por-jugada (como `battingLedger.js`), no este proxy simple.
