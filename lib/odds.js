...
// Exported so callers can pull a non-featured market (e.g. totals_1st_5_innings) out of a per-event
// odds response the same way parseOddsEvents does for the bulk h2h/totals markets.
export function extractMarket(event, marketKey) {
...
// Like findTotalsLine, but returns every point offered for this side instead of just the first
// (a market's outcomes commonly repeat the same side name once per alternate point/price pair —
// e.g. multiple "Over" entries at 6.5, 7.5, 8.5 with different prices).
export function findAllTotalsLines(totalsMarket, side) {
  if (!totalsMarket) return [];
  return totalsMarket.outcomes
    .filter(o => o.name === side)
    .map(o => ({ price: o.price, point: o.point }));
}
