...
import { mlbDateToday } from './mlb.js';
...
// Lists every game the other MLB-simulator project predicted for a given date, regardless of
// whether this bot ever sent a moneyline signal for that game — unlike getSimPrediction (a
// per-game, never-throws lookup used inline while scanning), this is for a standalone report of
// the whole day's simulations, so a real query failure should surface as an error like the rest
// of this file's list queries.
export async function getSimPredictionsForDate(db, date) {
  const { data, error } = await db
    .from('sim_predictions')
    .select('home_team, away_team, home_pitcher, away_pitcher, home_win_pct, away_win_pct, sims')
    .eq('date', date)
