...
import { CAREER_ERA_WEIGHT, MONEYLINE_CAREER_ERA_WEIGHT } from './signals.js';
import { HOME_FIELD_BONUS, RECORD_ADJUSTMENT_SCALE } from './signals.js';
...
import { applyThinSampleDampening, THIN_SAMPLE_STARTS_THRESHOLD, THIN_SAMPLE_EXTRA_SHRINK } from './signals.js';
import { applyStreakBonus, STREAK_BONUS_SCALE, STREAK_BONUS_CAP } from './signals.js';
...
test('teamWinProbability: HOME_FIELD_BONUS and RECORD_ADJUSTMENT_SCALE default to their current (2026-08-11 backtest-validated) values', () => {
  assert.equal(HOME_FIELD_BONUS, 0.09);
  assert.equal(RECORD_ADJUSTMENT_SCALE, 0.5);
});
...
test('teamWinProbability: even an elite home team is capped at a realistic ceiling, not near-certainty', () => {
  ...
  assert.ok(p <= 0.80); // TEAM_WIN_PROB_CLAMP_HI
});
...
test('teamWinProbability: even a terrible away team has a realistic floor, not near-zero', () => {
  ...
  assert.ok(p >= 0.20); // TEAM_WIN_PROB_CLAMP_LO
});
...
test('moneylineEstimate: shrinkage preserves direction (still favors the stronger home team)', () => {
  ...
  assert.ok(p > 0.5);
  assert.ok(p < 0.75); // shrinkage should pull it noticeably closer to 0.5 than the unshrunk log5 output
});

test('applyThinSampleDampening: leaves probability untouched when startsCount meets the threshold', () => {
  assert.equal(applyThinSampleDampening(0.65, THIN_SAMPLE_STARTS_THRESHOLD), 0.65);
  assert.equal(applyThinSampleDampening(0.65, 20), 0.65);
});

test('applyThinSampleDampening: leaves probability untouched when startsCount is unknown (null)', () => {
  assert.equal(applyThinSampleDampening(0.65, null), 0.65);
});

test('applyThinSampleDampening: shrinks toward 0.5 when startsCount is below the threshold', () => {
  // 0.5 + (0.65 - 0.5) * 0.7 = 0.605
  const dampened = applyThinSampleDampening(0.65, 2);
  assert.ok(Math.abs(dampened - 0.605) < 1e-9);
  assert.ok(dampened < 0.65);
  assert.ok(dampened > 0.5);
});

test('applyThinSampleDampening: defaults to THIN_SAMPLE_STARTS_THRESHOLD (5) and THIN_SAMPLE_EXTRA_SHRINK (0.7)', () => {
  assert.equal(THIN_SAMPLE_STARTS_THRESHOLD, 5);
  assert.equal(THIN_SAMPLE_EXTRA_SHRINK, 0.7);
});

test('moneylineEstimate: dampens confidence when the favored side\'s pitcher has a thin sample', () => {
  const base = { home: { recordWinPct: 0.7, startingPitcherEra: 2.80 }, away: { recordWinPct: 0.4, startingPitcherEra: 4.80 } };
  const withoutThinSample = moneylineEstimate(base);
  const withThinSample = moneylineEstimate({
    home: { ...base.home, startsCount: 2 }, // home is favored here, and has a thin sample
    away: { ...base.away, startsCount: 20 },
  });
  assert.ok(withThinSample < withoutThinSample);
  assert.ok(withThinSample > 0.5); // direction is preserved, just dampened
});

test('moneylineEstimate: does not dampen based on the UNDERDOG\'s starts count', () => {
  const base = { home: { recordWinPct: 0.7, startingPitcherEra: 2.80 }, away: { recordWinPct: 0.4, startingPitcherEra: 4.80 } };
  const withoutThinSample = moneylineEstimate(base);
  const underdogThinSample = moneylineEstimate({
    home: { ...base.home, startsCount: 20 },
    away: { ...base.away, startsCount: 2 }, // away is the underdog here, thin sample shouldn't matter
  });
  assert.equal(underdogThinSample, withoutThinSample);
});

test('applyStreakBonus: boosts confidence when the favored side is on the hotter streak', () => {
  const boosted = applyStreakBonus(0.6, 5, -2); // home favored (0.6>0.5), home on a 5-game win streak, away on a 2-game skid
  assert.ok(boosted > 0.6);
});

test('applyStreakBonus: reduces confidence when the UNDERDOG side is on the hotter streak', () => {
  const reduced = applyStreakBonus(0.6, -2, 5); // home favored, but away is the one on the hot streak
  assert.ok(reduced < 0.6);
});

test('applyStreakBonus: caps the adjustment instead of letting an extreme streak swing it unbounded', () => {
  const capped = applyStreakBonus(0.6, 20, -20, { scale: STREAK_BONUS_SCALE, cap: STREAK_BONUS_CAP });
  assert.ok(Math.abs(capped - (0.6 + STREAK_BONUS_CAP)) < 1e-9);
});

test('applyStreakBonus: treats missing streak values as 0 (no adjustment) instead of throwing', () => {
  assert.equal(applyStreakBonus(0.6, null, null), 0.6);
  assert.equal(applyStreakBonus(0.6, undefined, undefined), 0.6);
});

test('applyStreakBonus: defaults to STREAK_BONUS_SCALE (0.018) and STREAK_BONUS_CAP (0.15)', () => {
  assert.equal(STREAK_BONUS_SCALE, 0.018);
  assert.equal(STREAK_BONUS_CAP, 0.15);
});

test('moneylineEstimate: applies the streak bonus after thin-sample dampening', () => {
  const base = { home: { recordWinPct: 0.6, startingPitcherEra: 3.50 }, away: { recordWinPct: 0.4, startingPitcherEra: 4.50 } };
  const withoutStreak = moneylineEstimate(base);
  const withHotHomeStreak = moneylineEstimate({
    home: { ...base.home, streak: 8 },
    away: { ...base.away, streak: -3 },
  });
  assert.ok(withHotHomeStreak > withoutStreak);
});
...
test('calibrateProjectedTotal: defaults to TOTALS_CALIBRATION_SHRINK (0.5)', () => {
  assert.equal(TOTALS_CALIBRATION_SHRINK, 0.5);
  // 9.0 + (11.0 - 9.0) * 0.5 = 10.0
  assert.ok(Math.abs(calibrateProjectedTotal(11.0, 9.0) - 10.0) < 1e-9);
});
...
test('MONEYLINE_CAREER_ERA_WEIGHT: is a separate, much lower value than the totals-only CAREER_ERA_WEIGHT', () => {
  // Split apart 2026-08-11: moneyline wants far MORE career weight than totals tolerates, so a
  // lower weight-on-recent-form number here is correct, not a copy-paste of the shared constant.
  assert.equal(MONEYLINE_CAREER_ERA_WEIGHT, 0.15);
  assert.ok(MONEYLINE_CAREER_ERA_WEIGHT < CAREER_ERA_WEIGHT);
});
