...
test('selectAlternateLine: falls back to the richest (closest-from-below) option when nothing clears the target', () => {
  const chosen = selectAlternateLine([{ price: 1.20 }, { price: 1.30 }, { price: 1.46 }]);
  assert.equal(chosen.price, 1.46);
});

test('selectAlternateLine: a single candidate is returned regardless of its price', () => {
  const chosen = selectAlternateLine([{ price: 3.20 }]);
  assert.equal(chosen.price, 3.20);
  assert.equal(selectAlternateLine([{ price: 1.10 }]).price, 1.10);
});
