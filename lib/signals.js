...
// Validated against a full-season 2025 backtest split into two independent halves (Mar-Jun vs
// Jul-Sep, run #47, 2427 moneyline predictions, part of the 2026-08-11 model-improvement pass):
// a random-parameter search that required improvement in BOTH halves (not just in aggregate --
// aggregate-only search on a single month had ~75% of random parameter draws "win", almost pure
// noise) converged tightly on home field being worth roughly DOUBLE what was modeled. Confirmed
// with a clean univariate sweep at the final operating point: accuracy peaks at 0.08-0.10 and
// degrades on both sides (0.04 -> 54.1%, 0.09 -> 56.1%, 0.14 -> 54.8%), not just monotonically
// increasing (which would suggest a search-boundary artifact rather than a real optimum).
export const HOME_FIELD_BONUS = 0.09;
...
// How strongly recordWinPct pulls the win probability away from 0.5 -- same 2026-08-11 two-half
// backtest validation as HOME_FIELD_BONUS above, same shape (clean peak at 0.5, degrading on both
// sides: 0.3 -> 55.5%, 0.5 -> 56.1%, 0.7 -> 55.5%). Was a bare inline 0.3 before this pass; promoted
// to a named constant since it's now a deliberately-tuned value, not an arbitrary placeholder.
export const RECORD_ADJUSTMENT_SCALE = 0.5;

// The pre-existing 0.30-0.70 win-probability clamp came out equally validated a touch wider:
// 0.30-0.70 -> 55.7% full-season, 0.20-0.80 -> 56.0%, both halves improving together. Smaller
// effect than the two constants above, kept symmetric around 0.5 rather than as separate
// exported constants since there's no evidence either bound wants to move independently
// (teamProbLo alone was completely flat across 0.10-0.30 in isolation).
const TEAM_WIN_PROB_CLAMP_LO = 0.20;
const TEAM_WIN_PROB_CLAMP_HI = 0.80;
...
export function teamWinProbability({ recordWinPct, startingPitcherEra, isHome, offensiveFactor = 1.0 }) {
  ...
  const recentAdj = (recordWinPct - 0.5) * RECORD_ADJUSTMENT_SCALE;
  const homeAdj = isHome ? HOME_FIELD_BONUS : -HOME_FIELD_BONUS;
  return clamp(base + recentAdj + homeAdj, TEAM_WIN_PROB_CLAMP_LO, TEAM_WIN_PROB_CLAMP_HI);
}
...
// Extra shrink applied ONLY when the favored side's starting pitcher has very few real starts
// (gamesStarted===1 appearances) behind his season/recent ERA. Validated against backtest run
// #43/#44, zoomed into high-confidence picks only (|dist from 0.5|>=0.15, the kind that actually
// goes out as a live signal): favored pitchers with <5 real starts hit 53.1% (n=98) vs. 68.5%
// (n=92) for pitchers with 10-20 starts — a ~15pp gap, moderately significant (p≈0.03). NOT
// confirmed across the unrestricted sample (thin samples alone, without the high-confidence
// filter, showed no clean pattern) and NOT the same as career-ERA-gap (tested separately, no
// pattern found at all — deliberately not used here). Scoped narrowly to where the effect was
// actually measured: only kicks in when a pick is already confident AND the favored pitcher's
// sample is thin, not as a general pitcher-reliability adjustment.
export const THIN_SAMPLE_STARTS_THRESHOLD = 5;
export const THIN_SAMPLE_EXTRA_SHRINK = 0.7;

export function applyThinSampleDampening(prob, favoredStartsCount, { threshold = THIN_SAMPLE_STARTS_THRESHOLD, extraShrink = THIN_SAMPLE_EXTRA_SHRINK } = {}) {
  if (favoredStartsCount == null || favoredStartsCount >= threshold) return prob;
  return clamp(0.5 + (prob - 0.5) * extraShrink, 0.05, 0.95);
}

// Nudges confidence up when the favored side is ALSO on the hotter current win/loss streak (and
// down when the favored side's opponent is the hotter one instead) -- current streak, from
// computeStreakFromSchedule in mlb.js, positive = win streak, negative = loss streak. Validated
// one change at a time against the already-shipped model (2026-08-11): improved BOTH May 2025
// (53.28% -> 55.47%) and the rest of the season out of sample (54.22% -> 55.21%, run #50, 2016
// games with zero May dates) -- a real, held-up effect, not May-specific noise. Brier got
// modestly worse (0.2512 -> 0.2553 full season) since this is a blunt additive nudge, not a
// re-calibrated probability -- accuracy was the metric asked for here, and it moved for real.
export const STREAK_BONUS_SCALE = 0.018;
export const STREAK_BONUS_CAP = 0.15;

export function applyStreakBonus(prob, homeStreak, awayStreak, { scale = STREAK_BONUS_SCALE, cap = STREAK_BONUS_CAP } = {}) {
  const favoredHome = prob > 0.5;
  const streakDiff = (homeStreak ?? 0) - (awayStreak ?? 0);
  const signedDiff = favoredHome ? streakDiff : -streakDiff;
  const bonus = clamp(signedDiff * scale, -cap, cap);
  return clamp(prob + (favoredHome ? bonus : -bonus), 0.05, 0.95);
}
...
export function moneylineEstimate({ home, away }) {
  ...
  const calibrated = calibrateWinProbability(log5(pHome, pAway));
  const favoredStartsCount = calibrated > 0.5 ? home.startsCount : away.startsCount;
  const dampened = applyThinSampleDampening(calibrated, favoredStartsCount);
  return applyStreakBonus(dampened, home.streak, away.streak);
}
...
// a pitcher's career ERA (all regular seasons strictly before the current one), for TOTALS/F5
// runs projections. May/June backtests at 10/20/30/40/50% career weight showed totals bias
// drifting steadily WORSE (further from zero) as career weight rose past 20% — so this stops at
// 20% (0.8), the last point before that reversal. Re-measure via the backtest if ever revisited,
// same as LEAGUE_AVG_TOP_WEIGHTED_OPS above. When a pitcher has no prior season (rookie), this
// constant is unused for him — the caller skips the second blendEraEstimates call.
//
// Moneyline does NOT use this constant — see MONEYLINE_CAREER_ERA_WEIGHT below. They were
// originally the same shared constant, compromised at 0.8 because the two markets pulled in
// different directions; split apart 2026-08-11 once a full-season two-half-validated search
// (run #47) showed moneyline specifically wants a much lower value (far MORE career weight) than
// totals tolerates, confirmed with a clean univariate sweep (0.8 -> 54.5% full-season moneyline
// accuracy, 0.15 -> 56.1%, plateauing there rather than a boundary artifact -- see below).
export const CAREER_ERA_WEIGHT = 0.8;

// Career ERA weighting for MONEYLINE specifically (kept separate from CAREER_ERA_WEIGHT above,
// which totals/F5 still use). The full-season two-half search (run #47, 2026-08-11) consistently
// converged on a MUCH lower value than the shared 0.8 -- i.e. much MORE weight on career ERA, not
// less: a multi-season career sample is simply a far larger, more reliable estimate of a pitcher's
// true talent than a partial-season recent+season sample, and the moneyline market rewarded
// leaning on it much harder than totals does. A clean univariate sweep (holding other params at
// their own validated values) showed accuracy improving monotonically from 0.8 down through 0.15,
// then plateauing (0.15 -> 56.1%, 0.05 -> 56.2%, 0.0 -> 56.0%) rather than continuing to the
// search boundary -- 0.15 is picked from inside that plateau (85% career / 15% recent+season)
// rather than the literal best point found, to keep at least some responsiveness to a pitcher's
// current-season form instead of ignoring it entirely.
export const MONEYLINE_CAREER_ERA_WEIGHT = 0.15;
...
// Validated against the full 2025-season backtest (run #33, shrink=0.8): totals bias by
// projected-magnitude bucket was cleanly monotonic across ~2,400 predictions — low projections
// (<7.5 runs) came in UNDER the real total by +1.13 runs on average, high projections (10.5+) came
// in OVER by -1.82 — the same overconfidence shape already fixed for moneyline
// (calibrateWinProbability), never applied here. Shrinks the raw projection toward a "gravity
// point" (leagueAvgTotal) rather than a fixed number, so a park-adjusted gravity point (see
// projectedTotalRuns below) doesn't get shrunk back toward a neutral league average that wouldn't
// be correct for e.g. Coors Field.
//
// Re-measured against a live-signal audit (2026-08-11): the 0.8 shrink above left the same
// directional bias largely intact (+0.70 low-bucket / -1.12 high-bucket, run #43, n=2427; matched
// a live sample where Over signals won by only +0.75 runs of average margin vs. Under's -1.59).
// A grid search over shrink values against that same run's bucket bias found MAE/RMSE almost
// perfectly flat across the whole range (0.8 -> 3.648 MAE, 0.26 -> 3.604 MAE) -- day-to-day run
// totals are dominated by randomness the model can't capture either way, so stronger shrink costs
// essentially nothing in raw accuracy. 0.5 is the point where the bucket bias becomes roughly
// symmetric (+0.30 low / -0.24 high) without over-compressing into a near-constant prediction (the
// low end still has a real, if thin, population of games at 0.5; by 0.3 it's down to 2). Same
// pattern confirmed independently on totals_f5 in the same run. Re-measure via the backtest if
// ever revisited, same as MONEYLINE_CALIBRATION_SHRINK.
export const TOTALS_CALIBRATION_SHRINK = 0.5;
