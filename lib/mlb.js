...
// Current win/loss streak from the same schedule shape computeWinPctFromSchedule reads (most
// recent game first) -- positive = winning streak, negative = losing streak, 0 if no finished
// games yet. Deliberately shares one schedule fetch with computeWinPctFromSchedule (see callers in
// scan.js/backtest.js) instead of a separate API call or a persisted ledger: unlike batter-vs-hand
// splits (lib/battingLedger.js, needs real play-by-play MLB has no other way to get), win/loss
// streak is fully derivable from the schedule response already being fetched for the record.
export function computeStreakFromSchedule(scheduleResponse, teamId) {
  const finals = [];
  for (const d of scheduleResponse.dates || []) {
    for (const g of d.games || []) {
      if (g.status?.detailedState !== 'Final') continue;
  ...
  const mostRecentWin = finals[0].isWinner;
  let streak = 0;
  for (const g of finals) {
    if (g.isWinner !== mostRecentWin) break;
    streak += 1;
  }
  return mostRecentWin ? streak : -streak;
}
