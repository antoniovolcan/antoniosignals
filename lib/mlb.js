...
// pitcherFactor, adjustedInningsForEarlyHookRisk) — but those only clamp a RATIO derived from this
...
// Same rationale as MIN/MAX_REALISTIC_ERA above: a 1-2-start sample can produce an absurd raw K/9
// (e.g. a short outing with several Ks looks like 25-30 per 9 innings). No downstream consumer
// clamps this as tightly as the ERA ratio clamps do, so it's worth guarding at the source too.
const MIN_REALISTIC_K9 = 3.0;
const MAX_REALISTIC_K9 = 16.0;
...
export function extractStrikeoutsPer9(pitcherSeasonStatsResponse) {
  ...
  return clamp((strikeOuts * 9) / inningsPitched, MIN_REALISTIC_K9, MAX_REALISTIC_K9);
}
