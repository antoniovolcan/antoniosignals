...
import { parseOddsEvents, findTeamPrice, findTotalsLine, findAllTotalsLines } from './odds.js';
...
const ALTERNATE_TOTALS_FIXTURE = {
  bookmaker: 'draftkings',
  outcomes: [
    { name: 'Over', price: 1.30, point: 5.5 },
    { name: 'Under', price: 3.20, point: 5.5 },
    { name: 'Over', price: 1.46, point: 6.5 },
    { name: 'Under', price: 2.75, point: 6.5 },
    { name: 'Over', price: 1.60, point: 7.5 },
    { name: 'Under', price: 2.40, point: 7.5 },
  ],
};

test('findAllTotalsLines: returns every point/price offered for a side', () => {
  const lines = findAllTotalsLines(ALTERNATE_TOTALS_FIXTURE, 'Over');
  assert.deepEqual(lines, [
    { price: 1.30, point: 5.5 },
    { price: 1.46, point: 6.5 },
    { price: 1.60, point: 7.5 },
  ]);
});

test('findAllTotalsLines: returns an empty array when the market is missing', () => {
  assert.deepEqual(findAllTotalsLines(null, 'Over'), []);
});
