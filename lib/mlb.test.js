...
import { sumBullpenOutsFromBoxscore } from './mlb.js';
import { computeActiveILCount } from './mlb.js';
...
const BULLPEN_BOXSCORE_FIXTURE = {
  teams: {
    home: {
      team: { id: 135 },
      players: {
        ID1: { stats: { pitching: { gamesStarted: 1, outs: 11 } } }, // starter, excluded
        ID2: { stats: { pitching: { gamesStarted: 0, outs: 3 } } },
        ID3: { stats: { pitching: { gamesStarted: 0, outs: 5 } } },
        ID4: { stats: { pitching: {} } }, // appeared but no outs recorded (e.g. pinch runner edge case)
        ID5: { stats: { batting: {} } }, // position player, no pitching stats at all
      },
    },
    away: {
      team: { id: 119 },
      players: {
        ID6: { stats: { pitching: { gamesStarted: 1, outs: 15 } } },
        ID7: { stats: { pitching: { gamesStarted: 0, outs: 6 } } },
      },
    },
  },
};

test('sumBullpenOutsFromBoxscore: sums only relief-pitcher outs, excluding the starter', () => {
  assert.equal(sumBullpenOutsFromBoxscore(BULLPEN_BOXSCORE_FIXTURE, 135), 8);
});

test('sumBullpenOutsFromBoxscore: works for the away team too', () => {
  assert.equal(sumBullpenOutsFromBoxscore(BULLPEN_BOXSCORE_FIXTURE, 119), 6);
});

test('sumBullpenOutsFromBoxscore: returns 0 for a team not in the boxscore', () => {
  assert.equal(sumBullpenOutsFromBoxscore(BULLPEN_BOXSCORE_FIXTURE, 999), 0);
});

const TRANSACTIONS_FIXTURE = {
  transactions: [
    { person: { id: 1 }, date: '2025-05-01', description: 'Yankees placed DH Player One on the 10-day injured list.' },
    { person: { id: 2 }, date: '2025-05-03', description: 'Yankees placed RHP Player Two on the 15-day injured list.' },
    { person: { id: 1 }, date: '2025-05-10', description: 'Yankees transferred DH Player One from the 10-day injured list to the 60-day injured list.' },
    { person: { id: 3 }, date: '2025-05-12', description: 'Yankees optioned OF Player Three to Scranton/Wilkes-Barre.' }, // not IL-related, ignored
    { person: { id: 2 }, date: '2025-05-15', description: 'Yankees activated RHP Player Two from the 15-day injured list.' },
  ],
};

test('computeActiveILCount: counts players currently on the IL as of a cutoff date', () => {
  // As of 2025-05-16: player 1 still out (transferred to a longer IL, never activated), player 2
  // back (activated 05-15), player 3 never was IL-related.
  assert.equal(computeActiveILCount(TRANSACTIONS_FIXTURE, '2025-05-16'), 1);
});

test('computeActiveILCount: a transfer to a longer IL still counts as out, not a return', () => {
  assert.equal(computeActiveILCount(TRANSACTIONS_FIXTURE, '2025-05-11'), 2);
});

test('computeActiveILCount: only counts transactions strictly before the cutoff date (leak-free)', () => {
  assert.equal(computeActiveILCount(TRANSACTIONS_FIXTURE, '2025-05-01'), 0);
  assert.equal(computeActiveILCount(TRANSACTIONS_FIXTURE, '2025-05-02'), 1);
});

test('computeActiveILCount: returns 0 for a team with no transactions', () => {
  assert.equal(computeActiveILCount({ transactions: [] }, '2025-06-01'), 0);
});
