...
test('extractStrikeoutsPer9: clamps a small-sample blowup', () => {
  const k9 = extractStrikeoutsPer9({ stats: [{ splits: [{ stat: { strikeOuts: 5, inningsPitched: '1.0' } }] }] });
  // raw would be 5*9/1 = 45 -- must be clamped to the realistic 3.0-16.0 band
  assert.equal(k9, 16.0);
});
...
test('computeRecentStrikeoutsPer9: clamps a small-sample blowup', () => {
  const k9 = computeRecentStrikeoutsPer9({ stats: [{ splits: [{ date: '2025-04-01', stat: { strikeOuts: 6, inningsPitched: '1.0' } }] }] }, 5);
  // raw would be 6*9/1 = 54 -- must be clamped to the realistic 3.0-16.0 band
  assert.equal(k9, 16.0);
});
