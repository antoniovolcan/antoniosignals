...
import { filterGameLogBefore, computeWinPctFromSchedule, computeStreakFromSchedule } from './mlb.js';
...
test('computeStreakFromSchedule: positive streak for consecutive wins, most recent first', () => {
  const dates = [];
  for (let i = 1; i <= 5; i++) {
    const day = String(i).padStart(2, '0');
    dates.push({ date: `2026-05-${day}`, games: [{ status: { detailedState: 'Final' }, teams: { home: { team: { id: 147 }, isWinner: true }, away: { team: { id: 121 }, isWinner: false } } }] });
  }
  assert.equal(computeStreakFromSchedule({ dates }, 147), 5);
});

test('computeStreakFromSchedule: negative streak for consecutive losses', () => {
  const dates = [];
  for (let i = 1; i <= 3; i++) {
    const day = String(i).padStart(2, '0');
    dates.push({ date: `2026-05-${day}`, games: [{ status: { detailedState: 'Final' }, teams: { home: { team: { id: 147 }, isWinner: false }, away: { team: { id: 121 }, isWinner: true } } }] });
  }
  assert.equal(computeStreakFromSchedule({ dates }, 147), -3);
});

test('computeStreakFromSchedule: stops counting at the first result that breaks the streak', () => {
  const dates = [
    { date: '2026-05-01', games: [{ status: { detailedState: 'Final' }, teams: { home: { team: { id: 147 }, isWinner: true }, away: { team: { id: 121 }, isWinner: false } } }] }, // W (oldest)
    { date: '2026-05-02', games: [{ status: { detailedState: 'Final' }, teams: { home: { team: { id: 147 }, isWinner: false }, away: { team: { id: 121 }, isWinner: true } } }] }, // L
    { date: '2026-05-03', games: [{ status: { detailedState: 'Final' }, teams: { home: { team: { id: 147 }, isWinner: true }, away: { team: { id: 121 }, isWinner: false } } }] }, // W
    { date: '2026-05-04', games: [{ status: { detailedState: 'Final' }, teams: { home: { team: { id: 147 }, isWinner: true }, away: { team: { id: 121 }, isWinner: false } } }] }, // W (most recent)
  ];
  // most recent first: W, W, L, W -> current streak is 2 wins, stops at the L
  assert.equal(computeStreakFromSchedule({ dates }, 147), 2);
});

test('computeStreakFromSchedule: returns 0 when there are no finished games for the team', () => {
  assert.equal(computeStreakFromSchedule({ dates: [] }, 147), 0);
});
